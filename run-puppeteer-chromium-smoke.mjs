...
const chromiumRuntimeDir = mkdtempSync(join(tmpdir(), 'omux-puppeteer-chromium-'));
...
if (!existsSync(join(buildDir, 'index.html'))) {
	console.error('omux Puppeteer smoke requires build/index.html');
	process.exit(1);
}
...
try {
	...
	const mainText = await page.$eval('main', (element) => element.textContent ?? '');
	for (const term of ['oauth-mux', 'Install', 'View on GitHub']) {
		if (!mainText.includes(term)) {
			throw new Error(`omux Puppeteer smoke did not render expected main content: ${term}`);
		}
	}

	await page.goto(`${baseURL}/#install`, { waitUntil: 'networkidle0' });
	await page.waitForSelector('#install');
	const installText = await page.$eval('#install', (element) => element.textContent ?? '');
	if (!installText.includes('Install')) {
		throw new Error('omux Puppeteer smoke did not render the install section');
	}
} finally {
	await browser?.close();
