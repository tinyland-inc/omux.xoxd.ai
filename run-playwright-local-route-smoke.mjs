...
const chromiumRuntimeDir = mkdtempSync(join(tmpdir(), 'omux-playwright-local-route-'));
...
if (!existsSync(routeFile)) {
	console.error('omux Playwright local-route smoke requires build/agent-snippet.html');
	process.exit(1);
}
...
try {
	...
	for (const term of [
		...
		if (!bodyText.includes(term)) {
			throw new Error(`omux local route smoke did not render expected content: ${term}`);
		}
	}

	console.log(`omux Playwright local-route smoke passed with ${chromiumPath}`);
} finally {
	await browser?.close();
