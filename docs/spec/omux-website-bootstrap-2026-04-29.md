...
Issue context: Linear `TIN-734` (Publish oauth-mux website and launch narrative), child of `TIN-491` (Done). GitHub `tinyland-inc/omux.xoxd.ai`. Source tool: `Jesssullivan/oauth-mux` (public).

## Baseline

`omux.xoxd.ai` is a fresh repo (`tinyland-inc/omux.xoxd.ai`, currently a single
...
is `https://omux.xoxd.ai`; the canonical source repo is `Jesssullivan/oauth-mux`
...
- **Name**: `omux.xoxd.ai` (matches the deployed hostname; ergonomic for
  search and bookmark).
- **Visibility**: currently GitHub `INTERNAL`. Stay internal through M0
  bootstrap. Flip public at the same time as `omux.xoxd.ai` DNS goes live
  (start of M4).
...
- **Custom domain**: `omux.xoxd.ai`. Configure the repo Pages custom domain via
...
- **Redirects**:
  - `oauth-mux.xoxd.ai` → `omux.xoxd.ai` via Cloudflare Redirect Rule after the
...
Deliverables:
- Linear project `oauth-mux Website Launch (omux.xoxd.ai)` attached to
...
Goal: `https://omux.xoxd.ai` is live. Repo is public. README links to it.
Soft-launch posts ready (not yet distributed).

Deliverables:
- DNS CNAME record in Cloudflare
- Pages custom domain configured for `omux.xoxd.ai`; `static/CNAME` may be
  present as a single-line artifact but is not the source of truth
- Pages source set to `actions` via `gh api`
- Custom domain set to `omux.xoxd.ai` via `gh api`
...
- `oauth-mux/README.md` updated with link to `https://omux.xoxd.ai` (PR
...
Acceptance:
- `curl -sI https://omux.xoxd.ai/` returns 200 with `server: GitHub.com`
...
- **Project name**: `oauth-mux Website Launch (omux.xoxd.ai)`
- **Team**: `Tinyland`
...
4. **`oauth-mux.xoxd.ai` redirect**: default is a Cloudflare Redirect Rule to
   `https://omux.xoxd.ai` after the canonical Pages domain is healthy.
...
- Linear: `TIN-734` umbrella + new project `oauth-mux Website Launch
  (omux.xoxd.ai)` + M0-M5 milestones
- GitHub: `tinyland-inc/omux.xoxd.ai` (this repo) + `Jesssullivan/oauth-mux`
  (canonical source)
